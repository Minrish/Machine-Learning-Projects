...
def k_init(X, k):
    ...
    for i in range(k-1):
        ...
        ind = np.random.choice(np.arange(0, len(X)), p=x_dist) 
        centers.append(X.iloc[ind,:])
    return centers
    ...
    for x in range(len(X)):
        ...
        for c in C:
            ...
            if min_dist > dist:
                min_dist = dist
                cluster = c
        data_map.append((X.iloc[x,:], cluster))
    return data_map
...
# First we test run with k=1,2,3,4,5
acc = []
...
# Using k=5 as the final selection for our k-means
acc = []
...
# Finally, we plot with data colored by cluster
centers = k_means_pp(new_set, 5, 200)
